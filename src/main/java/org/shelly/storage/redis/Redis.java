package org.shelly.storage.redis;

import java.io.IOException;

import org.shelly.session.Serialize;
...
public class Redis {
	...
	public <T> T get(Class<T> clazz, String key) {
		if (cluster) {
			return valueFromBytes(clazz, jedisCluster.get(keyToBytes(key)));
		} else {
			Jedis jedis = getJedis();
			try {
				return valueFromBytes(clazz, jedis.get(keyToBytes(key)));
			} finally {
				close(jedis);
			}
		}
	...
	private <T> T valueFromBytes(Class<T> clazz, byte[] bytes) {
		if (bytes != null) {
			return Serialize.deserialize(clazz, bytes);
		}
		return null;
	}
